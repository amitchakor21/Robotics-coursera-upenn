...
for i = 2:nsamples
    ...
    for j = 1:min(k,length(distances))
        if(LocalPlanner(x,samples(:,index_sorted(j))) && distances_sorted(j)>0)
            nedges = nedges +1;
            edges(nedges,:)=[i,index_sorted(j)];
            edge_lengths(nedges) = distances_sorted(j);
        end
    end
